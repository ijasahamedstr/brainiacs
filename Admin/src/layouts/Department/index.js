...
function Department() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_HOST}/Department`);
        setCity(response.data); // Set the fetched categories
      } catch (error) {
  ...
  const columns = [
    ...
    { Header: "Department description", accessor: "Departmentdec", align: "left" },
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${process.env.REACT_APP_API_HOST}/Department/${id}`);
        setCity(City.filter((city) => city._id !== id)); // Update local state after deletion
  ...
  const rows = City.map((item) => ({
    Department: <Author name={item.department} />,
    Departmentdec: <Author name={item.departmentdec} />,
    BackgroundImage: (
      <MDBox>
        {item.imgpath ? (
          <img
            src={`${process.env.REACT_APP_API_HOST}/uploads/Department/${item.imgpath}`}
            alt="Background"
            style={{
