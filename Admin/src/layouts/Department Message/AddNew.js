...
function AddDepartmentMessage() {
  const [loading, setLoading] = useState(false);
  const [imagePreview, setImagePreview] = useState(null);
  const [lecturername, setlecturername] = useState("");
  const [lecturerdec, setLecturerdec] = useState("");
  ...
  const handleChange = (e) => {
    ...
    if (name === "lecturername") setlecturername(value);
    if (name === "lecturerdec") setLecturerdec(value);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // Basic validation
    if (!lecturername || !lecturerdec || !file) {
      Swal.fire({
        icon: "error",
        title: "Oops...",
    ...
    formData.append("lecturerdec", lecturerdec);
    try {
      const res = await axios.post(
        `${process.env.REACT_APP_API_HOST}/DepartmentMessage`,
        formData,
        {
...
export default AddDepartmentMessage;
